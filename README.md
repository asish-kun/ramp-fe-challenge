...
# Bug 1 Fix: Select dropdown doesn't scroll with rest of the page
## Solution
Modified `InputSelect` component for stable dropdown positioning:
1. In `src/components/InputSelect/index.tsx`:
   - Removed `setDropdownPosition` state and related logic
   - Updated dropdown container style:
     ```typescript
     style={{ position: 'absolute', width: '100%', zIndex: 1 }}
     ```
2. In `src/index.css`:
   - Added to `.RampInputSelect--root`:
     ```css
     position: relative;
...
# Bug 2 Fix: Approve checkbox not working
## Solution
Modified CSS to make checkboxes visible:
1. In `src/index.css`:
   - Updated the `.RampInputCheckbox--input` class:
     ```css
...
# Bug 3 Fix: Cannot select _All Employees_ after selecting an employee
## Solution
Modified `InputSelect` component in `App.tsx` to handle "All Employees" selection:
1. In `src/App.tsx`:
   - Updated the `onChange` handler in `InputSelect`:
     ```typescript
...
# Bug 4 Fix: Clicking on View More button not showing correct data
## Solution

Modified `usePaginatedTransactions` hook to properly append new transactions:
1. In `src/hooks/usePaginatedTransactions.ts`:
   - Updated the `setPaginatedTransactions` callback in `fetchAll`:
     ```typescript
...
# Bug 5 Fix: Employees filter not available during loading more data
## Solution
Modified `App` component to separate employee loading state from transaction loading:
1. In `src/App.tsx`:
   - Added new state for employee loading:
     ```typescript
     const [isEmployeeLoading, setIsEmployeeLoading] = useState(true)
     ```
   - Updated `useEffect` to handle employee loading:
     ```typescript
     useEffect(() => {
     ...
     ```
   - Updated `InputSelect` component props:
     ```typescript
...
# Bug 6 Fix: View more button not working as expected
## Solution
Modified App component to conditionally render the "View More" button:
1.In src/App.tsx:
   - Updated the `useEffect` to set `showViewMore` based on `paginatedTransactions` and `transactions`:
     ```typescript
     useEffect(() => {
       setShowViewMore(paginatedTransactions !== null && paginatedTransactions.nextPage !== null)
